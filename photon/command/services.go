...
func createService(c *cli.Context, w io.Writer) error {
	...
	master_vm_flavor := c.String("master-vm-flavor")
	worker_vm_flavor := c.String("worker-vm-flavor")
	disk_flavor := c.String("disk_flavor")
	subnet_id := c.String("subnet_id")
	...
	serviceSpec.SubnetId = subnet_id
...
func showService(c *cli.Context, w io.Writer) error {
	...
	if c.GlobalIsSet("non-interactive") {
		...
		if service.Type == "KUBERNETES" {
			...
			fmt.Println("  Harbor IP:           ", service.ExtendedProperties[photon.ExtendedPropertyMasterIPs])
		}
